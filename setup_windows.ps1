...
$ProjectPath = $PSScriptRoot  # Используем текущую папку как проект
$GoVersion = "1.23.4"
...
if (Test-WailsInstalled) {
    if (Test-Path "wails.json") {
        ...
        Write-Host "  ⊘ frontend найден. Для полной сборки Wails выполните: wails build" -ForegroundColor Gray
        Write-Host "  Текущая сборка работает в режиме go build (без встроенного UI)" -ForegroundColor Yellow
    } else {
